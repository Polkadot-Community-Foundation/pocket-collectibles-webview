...
export function ambientStar(pool: Pool, width: number, height: number): void {
  // Floating motes — spawn anywhere across the canvas with a gentle drift
  // in any direction. `fadeIn` ramps fresh motes from invisible to peak
  // alpha over the first ~12% of life so they don't pop in at the spawn
  // point; combined with the distributed spawn position there's no visible
  // edge of origin. Reads as ambient dust catching the light.
  const warm = Math.random() < 0.7
  spawn(pool, {
    x: rand(0, width),
    y: rand(0, height),
    ...
    life: rand(3.5, 6.0),
    r: 255,
    g: warm ? rand(220, 250) : rand(232, 250),
    b: warm ? rand(160, 210) : rand(225, 255),
    drag: 0.4,
    fadeIn: 0.12,
    blend: 'lighter',
    shape: 'circle'
