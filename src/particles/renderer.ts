...
export function createRenderer(canvas: HTMLCanvasElement, pool: Pool): Renderer {
  ...
  function tick(t: number): void {
    ...
    for (const p of pool) {
      ...
      p.x += p.vx * dt
      p.y += p.vy * dt
      // Tent alpha: linear fade-out by default; if fadeIn > 0, also ramp UP
      // over the first `fadeIn * max` seconds of life so ambient particles
      // wink in softly instead of popping at full alpha on spawn.
      const tA = p.life / p.max          // 1 at birth → 0 at death
      let a = tA
      if (p.fadeIn > 0) {
        const inDone = (1 - tA) / p.fadeIn
        if (inDone < a) a = inDone
      }
      if (a < 0) a = 0
      // outer soft
    ...
    for (const p of pool) {
      ...
      p.x += p.vx * dt
      p.y += p.vy * dt
      const tB = p.life / p.max
      let a = tB
      if (p.fadeIn > 0) {
        const inDone = (1 - tB) / p.fadeIn
        if (inDone < a) a = inDone
      }
      if (a < 0) a = 0
